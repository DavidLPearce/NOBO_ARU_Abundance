...
site <- as.numeric(factor(alfl$id))
...
# params <- c("p0", "alpha0", "alpha1", "alpha2", "beta0", "beta1", "psi", "lambda")
# 
# # Initial values
# inits <- function() {
#   list (p0 = runif(1), 
...
# Call nimble fr om R and summarize marginal posteriors
...
out_matrix <- as.matrix(out)
# Extract lambda samples
lambda_indices <- grep("^lambda", colnames(out_matrix))
lambda_samples <- out_matrix[, lambda_indices]

# Compute posterior summaries for each site
lambda_means <- colMeans(lambda_samples)  # Posterior means
lambda_cis <- apply(lambda_samples, 2, quantile, probs = c(0.025, 0.975))  # 95% credible intervals
...
# Compute total abundance for each iteration
total_abundance_samples <- rowSums(lambda_samples)

# Summarize total abundance
total_abundance_mean <- mean(total_abundance_samples)
total_abundance_ci <- quantile(total_abundance_samples, probs = c(0.025, 0.975))

# Display total abundance
cat("Total Abundance: Mean =", total_abundance_mean, 
    ", 95% CI =", total_abundance_ci, "\n")



# Plot site-specific abundance
plot(1:length(lambda_means), lambda_means, ylim = range(lambda_cis), pch = 16, 
     xlab = "Site", ylab = "Abundance")
arrows(1:length(lambda_means), lambda_cis[1,], 1:length(lambda_means), lambda_cis[2,], 
       length = 0.1, angle = 90, code = 3)

# Optional: Total abundance distribution
hist(total_abundance_samples, breaks = 30, main = "Posterior of Total Abundance", 
     xlab = "Total Abundance")



# Assuming total_abundance_samples is computed as:
total_abundance_samples <- rowSums(lambda_samples)
# Create a data frame
total_abundance_df <- data.frame(TotalAbundance = total_abundance_samples)

# Load ggplot2 if not already loaded
library(ggplot2)

# Create violin plot
ggplot(total_abundance_df, aes(x = "", y = TotalAbundance)) +
  geom_violin(fill = "skyblue", color = "black") +  # Violin layer
  geom_boxplot(width = 0.1, fill = "white", color = "black") +  # Optional: Add a boxplot overlay
  labs(title = "Posterior Distribution of Total Abundance",
       x = "",
       y = "Total Abundance") +
  theme_minimal()+
