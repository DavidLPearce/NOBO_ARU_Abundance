...
# Read in LiDAR data
las_folder <- "E:/Projects/Current_Projects/NOBO Abund S TX/GIS/LIDAR2018_70cm/LaCopita_LiDAR_tiles/LAS"# Set directory to las files
...
# Other metrics?
#   **** change to Cohesion index maybe?     landscapemetrics::calculate_lsm(lulc_clip, what = "lsm_c_cohesion")
#    Landscape division index DIVI, landscape shape index, splitting index
#    https://www.sciencedirect.com/science/article/pii/S0198971522001430
...
for (row in 1:NROW(site_dat)) {
  ...
  site_dat[row, "woody_IJI"] <- woody_c_iji[1, 'value']
  site_dat[row, "herb_IJI"] <- herb_c_iji[1, 'value']
  ...
  site_dat[row, "woody_COH"] <- woody_c_coh[1, 'value']
  site_dat[row, "herb_COH"] <- herb_c_coh[1, 'value']
  ...
  site_dat[row, "woody_DIV"] <- woody_c_div[1, 'value']
  site_dat[row, "herb_DIV"] <- herb_c_div[1, 'value']
  ...
  site_dat[row, "woody_SPLIT"] <- woody_c_split[1, 'value']
  site_dat[row, "herb_SPLIT"] <- herb_c_split[1, 'value']
