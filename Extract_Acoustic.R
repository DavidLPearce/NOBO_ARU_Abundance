...
# Adding a site number column
site_dat$SiteNum <- 1:27
...
#    Loop to extract
# ------------------------
for (index in 1:NROW(site_dat[,'SiteNum'])) {
  
    # Subset the site
  site_sub <- site_dat[which(site_dat[,'SiteNum'] == index),]
  ...
  site_dat[site_dat$SiteNum == index, "woody_prp"] <- woody_prp[1, 'layer']           # Proportion woody
  site_dat[site_dat$SiteNum == index, "herb_prp"] <- herb_prp[1, 'layer']             # Proportion herbaceous
  site_dat[site_dat$SiteNum == index, "baregnd_prp"] <- baregnd_prp[1, 'layer']       # Proportion bare ground
  site_dat[site_dat$SiteNum == index, "woody_mean_p_Area"] <- woody_p_area_mean       # Woody mean patch area
  site_dat[site_dat$SiteNum == index, "woody_c_clumpy"] <- woody_c_clumpy[1, 'value'] # Woody clumpy index
